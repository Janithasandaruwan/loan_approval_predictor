...
def generate_correlated_features(num_samples):
    ...
    education_level = np.random.choice(['High School', 'Associate', 'Bachelor', 'Master', 'Doctorate'], num_samples,
    ...
    credit_score = (credit_score_base + edu_factor * 100 + experience * 1.5 + income_noise * 100).clip(300, 850).astype(
        int)
    ...
    employment_status = np.array(['Employed', 'Self-Employed', 'Unemployed'])[
        np.argmax(np.random.random(num_samples)[:, np.newaxis] < employment_status_probs.cumsum(axis=1), axis=1)]
...
age, experience, education_level, annual_income, credit_score, employment_status = generate_correlated_features(
    num_samples)
...
data = {
    ...
    'LoanDuration': np.random.choice([12, 24, 36, 48, 60, 72, 84, 96, 108, 120], num_samples,
    ...
    'LoanPurpose': np.random.choice(['Home', 'Auto', 'Education', 'Debt Consolidation', 'Other'], num_samples,
...
df['BaseInterestRate'] = 0.03 + (850 - df['CreditScore']) / 2000 + df['LoanAmount'] / 1000000 + df[
    'LoanDuration'] / 1200
df['InterestRate'] = df['BaseInterestRate'] * (1 + np.random.normal(0, 0.1, num_samples)).clip(0.8, 1.2)

df['MonthlyLoanPayment'] = (df['LoanAmount'] * (df['InterestRate'] / 12)) / (
            1 - (1 + df['InterestRate'] / 12) ** (-df['LoanDuration']))
...
df.loc[noise_mask, 'AnnualIncome'] = (
            df.loc[noise_mask, 'AnnualIncome'] * np.random.uniform(1.5, 2.0, noise_mask.sum())).astype(int)
...
def assign_payment_history_risk(payment_history):
    if payment_history >= 99:
        return 1
    elif 97 <= payment_history < 99:
        return 2
    elif 95 <= payment_history < 97:
        return 3
    elif 90 <= payment_history < 95:
        return 4
    else:
        return 5
...
def assign_utilization_risk(utilization):
    if utilization < 0.20:
        return 1
    elif 0.20 <= utilization < 0.40:
        return 2
    elif 0.40 <= utilization < 0.60:
        return 3
    elif 0.60 <= utilization < 0.80:
        return 4
    else:
        return 5
...
def assign_credit_history_risk(length_of_history):
    if length_of_history >= 10:
        return 1
    elif 7 <= length_of_history < 10:
        return 2
    elif 5 <= length_of_history < 7:
        return 3
    elif 3 <= length_of_history < 5:
        return 4
    else:
        return 5
...
def assign_income_risk(annual_income):
    if annual_income >= 120000:
        return 1
    elif 80000 <= annual_income < 120000:
        return 2
    elif 50000 <= annual_income < 80000:
        return 3
    elif 30000 <= annual_income < 50000:
        return 4
    else:
        return 5
...
def assign_employment_risk(employment_status):
    if employment_status == 'Employed':
        ...
        return 4  # Unemployed or other


def assign_net_worth_risk(net_worth):
    if net_worth >= 500000:
        return 1
    elif 250000 <= net_worth < 500000:
        return 2
    elif 100000 <= net_worth < 250000:
        return 3
    elif 50000 <= net_worth < 100000:
        return 4
    else:
        return 5
...
print("\nFocused synthetic data saved to 'loan_data.csv'")
